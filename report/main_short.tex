...
The cognitive-severity assignments were robust to imputation under a fixed embedding and partition model. All ten imputation strategies met the pre-specified silhouette and noise criteria, although per-cluster bootstrap matching was only moderate and cluster count varied across methods. In the shared-pipeline H2 analysis, the average adjusted Rand index (ARI) between imputation strategies was 0.71. The primary multiple imputation chained equations (MICE) strategy had a full-coverage silhouette of 0.526 after noise reassignment. Tier assignment was associated with clinical unit (Cramér's $V=0.16$), but a patient-grouped multinomial sensitivity analysis did not show clear improvement in out-of-sample unit classification after tier was added to the broad diagnostic grouping. Additionally, domain-level aggregation reduced redundancy and improved numerical conditioning over variable-level clustering, decreasing mean variance inflation factor (VIF) from 2.8 to 1.9 and condition number from 62 to 12, while cluster separation changed only marginally. Most clustering and bootstrap analyses treated 17{,}406 assessments as observations rather than grouping them within 7{,}285 patients; repeated observations may therefore inflate apparent stability.



CogDash is an interactive dashboard developed to implement this entire pipeline for future patients. Additionally, it provides both severity tier assignments along with consensus confidence across all imputation methods. The code, notebooks and dashboard are accessible at \url{https://github.com/zkunos/thesis_project}.
\end{abstract}
