...
Finally, the pipeline imputes and then clusters, whereas model-based alternatives exist (such as latent-profile analysis via full-information maximum likelihood). The Gaussian-mixture baseline (Section~\ref{sec:robustness}) demonstrated that BIC decreased monotonically with increasing numbers of classes, providing support for a continuum rather than a discrete optimum. Thus, developing a latent-profile analysis that utilizes FIML instead of HDBSCAN is another viable area of future development.
