...
Each of the ten methods used in this dissertation will be presented in order of increasing complexity. Consider the data matrix X \in \mathbbR^n x p, which contains n assessments and p variables. Assume the i-th assessment includes p variables x_ij, and let \Omega = \{(i, j): x_ij  is observed\}. Then \Omega_j = \i : (i, j) \in \Omega\ represents the observed rows for the j-th variable. Define the operator P_\Omega as follows: for any entry x_ij of the matrix X, P_\Omega(x_ij) = x_ij, if x_ij \in \Omega; otherwise, P_\Omega(x_ij) = 0. The objective is to estimate \hatx_ij for entries not in \Omega.
...
Equation block:
\labeleq:mean
\hatx_ij=\barx_j=\frac1|\Omega_j|\sum_i\in\Omega_jx_ij.
...
Equation block:
\labeleq:mice
x_ij^(t)\sim p\left(x_ij\middle|x_i,-j^(t);\theta_j\right),\qquad j=1,\dots,p,
...
Equation block:
\labeleq:pmm
\hatx_ij\simUnif\bigl\{x_kj:k\in\mathcalD_ij\bigr\},\qquad
\mathcalD_ij=\bigl\{d observed k minimising |\hatx_kj-\hatx_ij|\bigr\}.
...
Equation block:
\labeleq:missforest
\hatx_ij=f_j\left(x_i,-j\right)\quaduntil\quad \sum_(i,j)\notin\Omega\bigl(x_ij^(t)-x_ij^(t-1)\bigr)^2  stops decreasing.
...
Equation block:
\labeleq:em
\hatx_mis=\boldsymbol\mu_mis+\Sigma_mis,obs\Sigma_obs,obs^-1\left(x_obs-\boldsymbol\mu_obs\right),
...
Equation block:
\labeleq:softimpute
\min_Z\ \tfrac12\bigl\lVert P_\Omega(X-Z)\bigr\rVert_F^2+lambda\lVert Z\rVert_*,


where S_lambda(Z)=Udiag\bigl((d_i-lambda)_+\bigr)V^\top. Thus, lambda determines what percentage of the variation in Z can be captured. Although SoftImpute is highly efficient for large sparse matrices, it could potentially ignore structure that does not fit within a low-rank model.
...
Equation block:
\labeleq:nmf
\min_W\ge0,H\ge0\ \bigl\lVert P_\Omega(X-WH)\bigr\rVert_F^2,\qquad W\in\mathbbR^nx r_\ge0,\ H\in\mathbbR^rx p_\ge0.
...
A denoising autoencoder (DAE) trains a neural network to predict complete data given intentionally degraded versions of that data under the MIDA framework (Gondara and Wang 2018). Using encoder f_\theta, decoder g_\theta, and corrupted input \tildex, the goal is to minimize the reconstruction error:


Equation block:
\labeleq:dae
\mathcalL_DAE(\theta)=\mathbbE\bigl\lVertx-g_\theta\left(f_\theta(\tildex)\right)\bigr\rVert^2.
...
Technically, UMAP works with a graph of weighted neighbors. For each pair of an object x_i and one of its neighboring objects x_j, there exists a directed membership probability denoted by p_j\mid i. In Equation <a href="#eq:umap_p" data-reference-type="ref" data-reference="eq:umap_p">[eq:umap_p]</a>, rho_i denotes the distance from x_i to its closest neighbor and sigma_i denotes a calibration factor such that \sum_j p_j\mid i=\log_2(k), where k is the value specified for `n_neighbors`. The directed probabilities are then made symmetric according to Equation <a href="#eq:umap_sym" data-reference-type="ref" data-reference="eq:umap_sym">[eq:umap_sym]</a>. Finally, in Equation <a href="#eq:umap_q" data-reference-type="ref" data-reference="eq:umap_q">[eq:umap_q]</a>, q_ij defines the low-dimensional membership probability, with a and b being functions of `min_dist`. Next, UMAP minimizes the cross-entropy in Equation <a href="#eq:umap_ce" data-reference-type="ref" data-reference="eq:umap_ce">[eq:umap_ce]</a> between the high-dimensional and low-dimensional memberships.


Equation block:
\labeleq:umap_p
p_j\mid i=\exp\left(-\frac{\max\0,d(x_i,x_j)-rho_i\}sigma_i\right),



Equation block:
\labeleq:umap_sym
p_ij=p_j\mid i+p_i\mid j-p_j\mid ip_i\mid j.



Equation block:
\labeleq:umap_q
q_ij=\left(1+a\lVert y_i-y_j\rVert_2^2b\right)^-1,



Equation block:
\labeleq:umap_ce
C=\sum_i!= j\left[p_ij\log\frac{p_ij}{q_ij}+(1-p_ij)\log\frac{1-p_ij}{1-q_ij}\right],
...
In addition to adjusting distance measurements based on local density, HDBSCAN also uses mutual reachability distance as defined in Equation <a href="#eq:mreach" data-reference-type="ref" data-reference="eq:mreach">[eq:mreach]</a> to connect pairs of objects. An MST is generated over these adjusted distances to form the basis for a hierarchical representation. Each node in this tree corresponds to either an object or a previously formed cluster. Persistent clusters are then selected based on their stability as described in Equation <a href="#eq:hdbscan_stab" data-reference-type="ref" data-reference="eq:hdbscan_stab">[eq:hdbscan_stab]</a>, where lambda_birth(C) represents the density level at which cluster C was created and lambda_\max(x,C) represents the highest density level at which point x remained part of cluster C. Any remaining points that do not belong to any persistent cluster are designated as noise.


Equation block:
\labeleq:mreach
d_mreach(a,b)=\max\bigl\core_k(a),core_k(b),d(a,b)\bigr\.



Equation block:
\labeleq:hdbscan_stab
S(C)=\sum_x\in C\bigl(lambda_\max(x,C)-lambda_birth(C)\bigr),\qquad lambda=\frac1{d_mreach},
...
Equation block:
\labeleq:ari
ARI=\frac{\displaystyle\sum_ij\binom{n_ij}2-\Bigl[\sum_i\binoma_i2\sum_j\binomb_j2\Bigr]\Big/\binomn2}{\displaystyle\tfrac12\Bigl[\sum_i\binoma_i2+\sum_j\binomb_j2\Bigr]-\Bigl[\sum_i\binoma_i2\sum_j\binomb_j2\Bigr]\Big/\binomn2}.



Equation block:
\labeleq:nmi
NMI(U,V)=\frac2I(U;V)H(U)+H(V),
...
Equation block:
\labeleq:hennig
\hatS_k=\frac1R\sum_r=1^R\max_k'J\left(C_k,C_k'^(r)\right).
...
Table entries:
- Cognitive Domain: Orientation; Variables: OPERSONA, OESPAI, OTEMPS; Count: 3
- Cognitive Domain: Attention; Variables: ASPAN, ATMTA; Count: 2
- Cognitive Domain: Visuoperception; Variables: VPIMATGES; Count: 1
- Cognitive Domain: Language; Variables: LREPETICIOTB, LDENOMINACIOTB, LCOMPRENSIOTB; Count: 3
- Cognitive Domain: Memory; Variables: MDIGITS, MRAVLT075, MRAVLT015, MRAVLT015R; Count: 4
- Cognitive Domain: Executive Function; Variables: FEPMR; Count: 1
- Cognitive Domain: Total; Count: 14
...
m_i\sim
\begincases
P_miss & with probability  0.5,\\[2pt]
Bernoulli(1-rho)^\otimes p & with probability  0.5,
\endcases


where P_miss denotes the empirical distribution of missingness patterns and rho denotes the uniform corruption rate. This scheme allows the DAE to learn from random corruption as well as from missingness structures that resemble those observed in the clinical data.
...
Equation block:
\labeleq:beta_cyclical
\beta(t)=\beta_\max\fract \bmod TT,\qquad T=40,\ \beta_\max=0.1,
...
Equation block:
\labeleq:rmse
RMSE=\sqrt{\frac1|H|\sum_(i,j)\in H\bigl(\hatx_ij-x_ij\bigr)^2},\qquad
MAE=\frac1|H|\sum_(i,j)\in H\bigl|\hatx_ij-x_ij\bigr|.




Table entries:
- Category: Conventional; Method: Mean; Year: —; k: 4; Sil.: 0.604; Advantages: Simplest; fast; universal baseline; Limitations: Collapses variance & covariance
- Category: Statistical; Method: EM; Year: 1977; k: 4; Sil.: 0.587; Advantages: Principled MLE; preserves covariance; Limitations: Assumes multivariate normality
- Method: MICE; Year: 2011; k: 3; Sil.: 0.598; Advantages: Flexible; preserves distribution; Limitations: Requires model per variable
- Category: Machine; Method: KNN; Year: 2001; k: 4; Sil.: 0.576; Advantages: Non-parametric; captures local structure; Limitations: Sensitive to k; slow for large n
- Category: Learning; Method: MissForest; Year: 2012; k: 3; Sil.: 0.606; Advantages: Non-linear; handles mixed types; Limitations: Computationally expensive
- Category: Hybrid; Method: PMM; Year: —; k: 4; Sil.: 0.498; Advantages: Preserves marginal distribution; Limitations: Requires adequate donor pool
- Category: Matrix; Method: SoftImpute; Year: 2010; k: 4; Sil.: 0.571; Advantages: Efficient for sparse matrices; Limitations: Assumes low-rank structure
- Category: Completion; Method: NMF; Year: 1999; k: 3; Sil.: 0.546; Advantages: Non-negativity natural for scores; Limitations: Sensitive to rank choice
- Category: Deep; Method: DAE; Year: 2018; k: 5; Sil.: 0.510; Advantages: Non-linear; multiple imputations; Limitations: Requires hyperparameter tuning
- Category: Learning; Method: VAE; Year: 2014; k: 4; Sil.: 0.603; Advantages: Probabilistic; principled uncertainty; Limitations: Complex training; beta-tuning
...
Table entries:
- Parameter: n_neighbors; Values: 15, 30, 50
- Parameter: min_dist; Values: 0.0
- Parameter: n_components; Values: 3, 5, 8
...
Equation block:
Q = silhouette x (1 - noise proportion)
\labeleq:quality
...
Table entries:
- Parameter: min_cluster_size; Values: 500, 1000, 1500, 2000, 3000
- Parameter: min_samples; Values: 5, 10, 25, 50
- Parameter: cluster_selection_method; Values: eom, leaf
- Parameter: cluster_selection_epsilon; Values: 0.0, 0.1, 0.5
...
Equation block:
\labeleq:cohend
d=\frac{\barx_1-\barx_2}s_p,\qquad s_p=\sqrt{\frac(n_1-1)s_1^2+(n_2-1)s_2^2n_1+n_2-2},
...
Equation block:
\labeleq:noise
noise=\frac1n\bigl|\i:label(i)=noise\\bigr|,
...
Equation block:
\labeleq:consensus
conf(i)=\max_\ell\frac1M\sum_m=1^M1\left[label_m(i)=\ell\right],


High-confidence assessments are those whose assessments were placed into the mode cluster by at least M/10 = 0.8 x 10 = 8 methods.

**Chi-Square Test, Cramér’s V, CMH Test**. Contingency tables arise from cross-classifying two categorical variables - e.g., cluster assignment and clinical unit - giving joint counts. Pearson’s Chi-Square Test evaluates observed counts O_ij against expected counts E_ij under the null hypothesis that the two variables are independent:


Equation block:
\labeleq:chi2
chi^2=\sum_i=1^r\sum_j=1^c\frac{(O_ij-E_ij)^2}{E_ij},\qquad E_ij=\frac{O_i\cdotO_\cdot j}n,


with O_i\cdot and O_\cdot j denoting row sums and column sums, respectively. The test statistic is referenced against a Chi-Square distribution with (r - 1)(c - 1) degrees of freedom to yield the p-value. Cramér’s V is used to render association statistics scale-free for sample-size purposes, since it takes on values between zero and one:


Equation block:
\labeleq:cramersv
V=\sqrt{\fracchi^2n(k-1)},\qquad k=\min(r,c),
...
Equation block:
\labeleq:cmh
chi^2_CMH=\frac{\bigl(\sum_h(a_h-\mathbbE[a_h])\bigr)^2}{\sum_hVar(a_h)}.
...
To test this prediction formally, we employed a Chi-square Test of Independence. **Chi Square Tests:** chi^2 = 911.3, p-value approx. 4 x 10^-156, Cramér’s V = 0.162.
...
Hypothesis 2 is also supported, with a defined scope. The primary tier solution was obtained with UMAP+HDBSCAN, but the imputation-sensitivity test used a secondary fixed-reference diagnostic: a common StandardScaler, UMAP embedding, and k-means partition fitted on MICE with k=3. Under that fixed labelling rule, the average Adjusted Rand Index across all forty-five pairwise method comparisons is 0.710 (95% bootstrap CI \[0.704, 0.716\]), greater than the 0.500 threshold. This establishes that point assignments are robust to imputation under a fixed embedding and partition. It does not imply that independently refitted HDBSCAN pipelines recover an identical density hierarchy or tier count; the full UMAP+HDBSCAN runs vary from k=3 to k=5, and the Rubin-pooled multiple-imputation draws vary from k=3 to k=7, consistent with discretising a continuum. Following Holm-Bonferroni adjustment, forty-four of forty-five pairwise combinations reject H_0: ARI <= 0.5 at alpha = 0.05. Only KNN-SoftImpute is borderline. Consensus clustering achieves a mean confidence of 0.929, with 86.2 percent of assessments receiving high-confidence assignment. The baseline comparison with listwise deletion verifies that imputation is analytically required: removing missing values from 46.9 percent of the assessments alters the number of clusters and lowers the silhouette value to 0.257.

Hypothesis 3 is supported as well. Clinical Unit is significantly correlated with cognitive severity tier (chi^2 = 911.3, p approx. 4 x 10^-156); however, the magnitude of the correlation is small (Cramér’s V = 0.162). The Cochran-Mantel-Haenszel statistic demonstrates that clinical unit is not solely determined by diagnosis group (Cochran 1954). Thus, cognitive severity appears to contribute to clinical unit designation independently of diagnostic category.
...
Table entries:
- Method: Mean; Hyperparameters: Single-pass arithmetic mean per variable.
- Method: EM; Hyperparameters: Multivariate normal model; convergence 10^-6; max 500 iterations.
- Method: MICE; Hyperparameters: Linear regression per variable; 10 iterations; single imputed dataset for clustering.
- Method: KNN; Hyperparameters: k = 5; inverse-distance weighting; Euclidean distance.
- Method: MissForest; Hyperparameters: 100 trees per forest; up to 10 iterations; variables imputed in order of increasing missingness.
- Method: PMM; Hyperparameters: 5 donors; 10 iterations; implemented within the MICE framework.
- Method: SoftImpute; Hyperparameters: Regularisation chosen by cross-validation over 20 log-spaced values from 10^-3 to 10^2; convergence 10^-5; max 200 iterations.
- Method: NMF; Hyperparameters: Rank r = 5 selected by grid search over 2, 3, 5, 7, 10; multiplicative updates; max 500 iterations.
- Method: DAE; Hyperparameters: Architecture 15 to 128 to 64 to 32 to 64 to 128 to 15; ReLU + batch normalisation; dropout (0.5, 0.3); corruption rate 20% with pattern-aware masking; MSE loss on observed entries; Adam optimiser (lr = 10^-3); ReduceLROnPlateau + early stopping (patience 20); 5 runs averaged.
- Method: VAE; Hyperparameters: Same layer dimensions as DAE; latent dim 32; loss = MSE + beta-weighted KL divergence; cyclical annealing with beta_max = 0.1 over 40-epoch cycles; K = 20 latent samples averaged for final imputation.
